...
# Filtering data for Coastal Giant Salamander
salamander_data <- and_vertebrates %>%
  filter(species == "Coastal giant salamander")

# Creatinh a contingency table for section (forest type) and unittype (channel unit type)
...
**A chi-square test of independence was conducted to examine the relationship between forest type (section) and channel unit (unittype) for Coastal Giant Salamander abundance. The test revealed a statistically significant association between the two variables (X² = 200.71, df = 5, p \< 0.001), indicating that the distribution of salamanders varies significantly across different channel units based on the forest type.**
...
salamander_data <- and_vertebrates %>%
  filter(species == "Coastal giant salamander") %>%
  filter(!is.na(weight_g))  # Remove NA values in weight

# Checking sample size for each forest type
table(salamander_data$section)

# Checking for equal variance using an F-test
var_test <- var.test(weight_g ~ section, data = salamander_data)
print(var_test)

# Performing t-test: Use Welch’s t-test if variances are unequal
if (var_test$p.value < 0.05) {
  t_test_result <- t.test(weight_g ~ section, data = salamander_data, var.equal = FALSE)
} else {
  t_test_result <- t.test(weight_g ~ section, data = salamander_data, var.equal = TRUE)
}
...
**\*\*The test revealed a statistically significant difference between the two groups (t = 4.93, df = 6335.9, p \< 0.001).\
**

**The mean biomass was higher in clear-cut forests (M = 9.81 g, SD ≠ OG variance) compared to old-growth forests (M = 8.32 g).\
**

**The 95% confidence interval \[0.90, 2.09\] confirms that this difference is unlikely due to random chance.\
**

**These results suggest that forest type significantly affects the biomass of Coastal Giant Salamanders.\*\***
...
# Filtering for Cutthroat Trout and remove missing values in length and weight
trout_data <- and_vertebrates %>%
  filter(species == "Cutthroat trout") %>%
  filter(!is.na(length_1_mm) & !is.na(weight_g))

# Taking a random subset (max 5000 values) for the normality test
trout_sample <- trout_data %>% sample_n(min(5000, nrow(trout_data)))

# Performing normality test on a subset
shapiro_length <- shapiro.test(trout_sample$length_1_mm)
shapiro_weight <- shapiro.test(trout_sample$weight_g)

# Determining correlation method based on normality results
if (shapiro_length$p.value > 0.05 & shapiro_weight$p.value > 0.05) {
  cor_method <- "pearson"
} else {
  cor_method <- "spearman"
}
...
cor_test <- cor.test(trout_data$length_1_mm, trout_data$weight_g, method = cor_method)
...
ggplot(trout_data, aes(x = length_1_mm, y = weight_g)) +
  ...
  labs(
    title = "Correlation Between Body Length and Weight in Cutthroat Trout",
    x = "Body Length (mm)",
    y = "Body Weight (g)"
  ) +
  theme_minimal()
...
**\*\*The results indicate an extremely strong positive correlation (rho = 0.992, p \< 0.001), suggesting that longer trout tend to weigh significantly more.\
**

**Since the p-value is highly significant, we conclude that body length and weight are strongly associated in Cutthroat Trout.\*\***
...
```{r}
# Filtering a subset of the sites
pie_sites <- pie_crab |> 
...
# Selecting three sites for analysis
selected_sites <- pie_crab %>%
  filter(site %in% c("GTM", "DB", "PIE"))

# Checking the order of latitude for the selected sites
selected_sites %>%
  group_by(site) %>%
  summarise(mean_latitude = mean(latitude))

# Ensuring sites are ordered by latitude
selected_sites$site <- factor(selected_sites$site, 
                              levels = selected_sites %>%
                                group_by(site) %>%
                                summarise(mean_latitude = mean(latitude)) %>%
...
leveneTest(size ~ site, data = selected_sites)

# Performing ANOVA
anova_result <- aov(size ~ site, data = selected_sites)
...
```{r}
# Create boxplot with sites ordered by latitude
ggplot(selected_sites, aes(x = site, y = size, fill = site)) +
  geom_boxplot() +
  labs(title = "Crab Carapace Width Across Sites",
...
**The results indicated a statistically significant effect of site on carapace width (F = 60.02, df = 2, 83, p \< 0.001).\
A Tukey’s HSD post-hoc test revealed that:**

-   **DB vs. GTM (p \< 0.001) → Significant difference**

-   **PIE vs. GTM (p \< 0.001) → Significant difference These findings suggest that crabs from different sites have significantly different carapace sizes, likely influenced by environmental factors such as temperature or habitat conditions.**
    ...
    linear_model <- lm(size ~ water_temp_sd, data = pie_crab)
    ...
    ggplot(pie_crab, aes(x = water_temp_sd, y = size)) +
      geom_point(alpha = 0.5, color = "blue") +  # Scatter points
      geom_smooth(method = "lm", se = TRUE, color = "red") +  # Regression line
      labs(title = "Effect of Water Temperature Variation on Crab Size",
    ...
    \*\*The results indicate no significant relationship between the two variables (β = 0.099, t = 0.632, p = 0.528).\

    The model explained only 0.1% of the variation in crab size (`R² = 0.0010`), indicating that water temperature variation does not significantly affect carapace width.\*\*
...
mlr_model <- lm(size ~ latitude + air_temp_sd + water_temp_sd, data = pie_crab)
...
The overall model was **statistically significant** (**F = 70.13, p \< 0.001**), explaining **35.2% of the variation in carapace width (`R² = 0.3516`)**.\

However, only **latitude had a significant effect** on carapace width (**β = 0.559, p \< 0.001**), indicating that **crabs at higher latitudes tend to be larger**.\

The effects of **air temperature variability (`p = 0.173`)** and **water temperature variability (`p = 0.325`)** were **not statistically significant**, suggesting that **temperature fluctuations do not strongly influence crab size**.
